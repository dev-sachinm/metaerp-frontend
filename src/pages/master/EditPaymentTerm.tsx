...
import { usePaymentTerm } from '@/hooks/graphql/useMasterDataQueries'
import { useUpdatePaymentTerm, type PaymentTermInput } from '@/hooks/graphql/useMasterDataMutations'
...
const DATA_KEY = 'paymentTerm' as const

export function EditPaymentTerm() {
  const navigate = useNavigate()
  const { id } = useParams<{ id: string }>()
  const entityId = id ?? ''
  const { data, isLoading, isError, error } = usePaymentTerm(entityId || null)
  const recordData = data as { paymentTerm: PaymentTerm | null } | undefined
  ...
  const onSubmit = async (values: Record<string, unknown>) => {
    ...
    try {
      ...
      await update.mutateAsync({ id: entityId, input: input as unknown as PaymentTermInput })
      logger.info('Payment term updated', { category: 'business', data: { entity: ENTITY, id: entityId } })
      navigate(LIST_PATH, { replace: true })
    } catch (err: unknown) {
      logger.error('Update payment term failed', { category: 'technical', error: err })
      setFormError(getErrorMessage(err, 'Failed to update payment term'))
    }
  }
  ...
  if (isError && error && isPermissionError(error)) {
    return (<DashboardLayout><div className="p-8 max-w-2xl mx-auto"><OperationNotPermitted context="You do not have permission to view or edit this payment term." /></div></DashboardLayout>)
  }
    ...
    return (
      <DashboardLayout>
        <div className="p-8 max-w-2xl mx-auto">
          <Card>
            <CardHeader>
              <CardTitle>{config?.listTitle ?? 'Payment Term'} not found</CardTitle>
          ...
          <Card className="overflow-hidden border-slate-200/80 shadow-sm">
            <CardHeader className="border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white pb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle className="text-xl font-semibold tracking-tight text-slate-900">{config?.editTitle ?? 'Edit Payment Term'}</CardTitle>
                  <CardDescription className="mt-1 text-sm text-slate-600">Update payment term details.</CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="pt-6">
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                  ...
                  {recordData[DATA_KEY] && ((recordData[DATA_KEY] as PaymentTerm).createdAt != null || (recordData[DATA_KEY] as PaymentTerm).modifiedAt != null) && (
                    <section className="rounded-lg border border-slate-200 bg-slate-50/50 px-4 py-3 space-y-1">
                      ...
                      <div className="flex flex-wrap gap-4 text-sm text-slate-600">
                        {(recordData[DATA_KEY] as PaymentTerm).createdAt != null && <span>Created: {new Date((recordData[DATA_KEY] as PaymentTerm).createdAt!).toLocaleString()}</span>}
                        {(recordData[DATA_KEY] as PaymentTerm).modifiedAt != null && <span>Modified: {new Date((recordData[DATA_KEY] as PaymentTerm).modifiedAt!).toLocaleString()}</span>}
                      </div>
                    </section>
                  )}
