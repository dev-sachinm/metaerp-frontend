...
import { useProduct, useProductCategories, useUOMList } from '@/hooks/graphql/useMasterDataQueries'
import { useUpdateProduct, type ProductInput } from '@/hooks/graphql/useMasterDataMutations'
...
const DATA_KEY = 'product' as const
...
export function EditProduct() {
  ...
  const recordData = data as { product: Product | null } | undefined
  ...
  const canEditCategory = useCanEditField(ENTITY, 'categoryId')
  const canEditUnit = useCanEditField(ENTITY, 'unitId')
  const { data: categoriesData, isLoading: categoriesLoading } = useProductCategories(0, 1000)
  const categoryOptions = categoriesData?.productCategories?.items ?? []
  const { data: uomData, isLoading: uomLoading } = useUOMList(0, 1000)
  const uomOptions = uomData?.uomList?.items ?? []
  ...
  const onSubmit = async (values: Record<string, unknown>) => {
    ...
    try {
      ...
      await update.mutateAsync({ id: entityId, input: input as unknown as ProductInput })
      logger.info('Product updated', { category: 'business', data: { entity: ENTITY, id: entityId } })
      navigate(LIST_PATH, { replace: true })
    } catch (err: unknown) {
      logger.error('Update product failed', { category: 'technical', error: err })
  ...
  if (isError && error && isPermissionError(error)) {
    return (<DashboardLayout><div className="p-8 max-w-2xl mx-auto"><OperationNotPermitted context="You do not have permission to view or edit this product." /></div></DashboardLayout>)
  }
    ...
    return (
      <DashboardLayout>
        <div className="p-8 max-w-2xl mx-auto">
          <Card>
            <CardHeader>
              <CardTitle>{config?.listTitle ?? 'Product'} not found</CardTitle>
      ...
      <div className="p-8 max-w-2xl mx-auto">
        <div className="mb-6">
          <Button variant="ghost" size="sm" onClick={() => navigate(LIST_PATH)} className="gap-2 text-slate-600 hover:text-slate-900">
            <ArrowLeft className="h-4 w-4" /> Back to {config?.listTitle ?? 'Products'}
          </Button>
        </div>
          ...
          <Card className="overflow-hidden border-slate-200/80 shadow-sm">
            <CardHeader className="border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white pb-6">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-indigo-100 text-indigo-600"><Package className="h-5 w-5" /></div>
                <div>
                  <CardTitle className="text-xl font-semibold tracking-tight text-slate-900">{config?.editTitle ?? 'Edit Product'}</CardTitle>
                  <CardDescription className="mt-1 text-sm text-slate-600">Update product details.</CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="pt-6">
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                  ...
                  <FieldGuard entity={ENTITY} fieldName="categoryId" action="read">
                    <FormField
                        ...
                        <FormItem>
                          ...
                          <FormControl>
                            <select
                              ...
                              disabled={categoriesLoading || !canEditCategory}
                            >
                  ...
                  <FieldGuard entity={ENTITY} fieldName="unitId" action="read">
                    <FormField
                      control={form.control}
                      name={'unitId'}
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <select
                              ...
                              value={(field.value as string) ?? ''}
                              onChange={(e) => field.onChange(e.target.value || null)}
                              disabled={uomLoading || !canEditUnit}
                            >
                              <option value="">{uomLoading ? 'Loading…' : 'Select unit'}</option>
                              {uomOptions.map((uom) => (
                  ...
                  {recordData[DATA_KEY] && ((recordData[DATA_KEY] as Product).createdAt != null || (recordData[DATA_KEY] as Product).modifiedAt != null) && (
                    <section className="rounded-lg border border-slate-200 bg-slate-50/50 px-4 py-3 space-y-1">
                      ...
                      <div className="flex flex-wrap gap-4 text-sm text-slate-600">
                        {(recordData[DATA_KEY] as Product).createdAt != null && <span>Created: {new Date((recordData[DATA_KEY] as Product).createdAt!).toLocaleString()}</span>}
                        {(recordData[DATA_KEY] as Product).modifiedAt != null && <span>Modified: {new Date((recordData[DATA_KEY] as Product).modifiedAt!).toLocaleString()}</span>}
                      </div>
                    </section>
                  )}
