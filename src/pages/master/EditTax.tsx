...
import { ArrowLeft, AlertCircle, Percent } from 'lucide-react'
...
import { useTax } from '@/hooks/graphql/useMasterDataQueries'
import { useUpdateTax, type TaxInput } from '@/hooks/graphql/useMasterDataMutations'
...
const ENTITY = 'tax'
const LIST_PATH = '/master/tax'
const DATA_KEY = 'tax' as const

export function EditTax() {
  const navigate = useNavigate()
  const { id } = useParams<{ id: string }>()
  const entityId = id ?? ''
  const { data, isLoading, isError, error } = useTax(entityId || null)
  const recordData = data as { tax: Tax | null } | undefined
  ...
  const onSubmit = async (values: Record<string, unknown>) => {
    ...
    try {
      ...
      await update.mutateAsync({ id: entityId, input: input as unknown as TaxInput })
      logger.info('Tax updated', { category: 'business', data: { entity: ENTITY, id: entityId } })
      navigate(LIST_PATH, { replace: true })
    } catch (err: unknown) {
      logger.error('Update tax failed', { category: 'technical', error: err })
      setFormError(getErrorMessage(err, 'Failed to update tax'))
    }
  }
  ...
  if (isError && error && isPermissionError(error)) {
    return (<DashboardLayout><div className="p-8 max-w-2xl mx-auto"><OperationNotPermitted context="You do not have permission to view or edit this tax." /></div></DashboardLayout>)
  }
    ...
    return (
      <DashboardLayout>
        <div className="p-8 max-w-2xl mx-auto">
          <Card>
            <CardHeader>
              <CardTitle>{config?.listTitle ?? 'Tax'} not found</CardTitle>
      ...
      <div className="p-8 max-w-2xl mx-auto">
        <div className="mb-6">
          <Button variant="ghost" size="sm" onClick={() => navigate(LIST_PATH)} className="gap-2 text-slate-600 hover:text-slate-900">
            <ArrowLeft className="h-4 w-4" /> Back to {config?.listTitle ?? 'Tax'}
          </Button>
        </div>
          ...
          <Card className="overflow-hidden border-slate-200/80 shadow-sm">
            <CardHeader className="border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white pb-6">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-indigo-100 text-indigo-600"><Percent className="h-5 w-5" /></div>
                <div>
                  <CardTitle className="text-xl font-semibold tracking-tight text-slate-900">{config?.editTitle ?? 'Edit Tax'}</CardTitle>
                  <CardDescription className="mt-1 text-sm text-slate-600">Update tax details.</CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="pt-6">
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                  ...
                  {recordData[DATA_KEY] && ((recordData[DATA_KEY] as Tax).createdAt != null || (recordData[DATA_KEY] as Tax).modifiedAt != null) && (
                    <section className="rounded-lg border border-slate-200 bg-slate-50/50 px-4 py-3 space-y-1">
                      ...
                      <div className="flex flex-wrap gap-4 text-sm text-slate-600">
                        {(recordData[DATA_KEY] as Tax).createdAt != null && <span>Created: {new Date((recordData[DATA_KEY] as Tax).createdAt!).toLocaleString()}</span>}
                        {(recordData[DATA_KEY] as Tax).modifiedAt != null && <span>Modified: {new Date((recordData[DATA_KEY] as Tax).modifiedAt!).toLocaleString()}</span>}
                      </div>
                    </section>
                  )}
