...
import { useCreateUOM, type UOMInput } from '@/hooks/graphql/useMasterDataMutations'
import {
  getEntityConfig,
  useEntityFormFields,
  EntityFormFields,
...
function buildInput(formFields: { key: string; type: string; readOnlyInForm?: boolean }[], data: Record<string, unknown>) {
...
export function CreateUOM() {
  ...
  const onSubmit = async (data: Record<string, unknown>) => {
    setFormError(null)
    try {
      const input = buildInput(formFields, data)
      await create.mutateAsync(input as unknown as UOMInput)
      logger.info('UOM created', { category: 'business', data: { entity: ENTITY, code: input.code } })
      navigate(LIST_PATH, { replace: true })
    } catch (err: unknown) {
      logger.error('Create UOM failed', { category: 'technical', error: err })
      setFormError(getErrorMessage(err, 'Failed to create UOM'))
    }
  }
          ...
          <Card className="overflow-hidden border-slate-200/80 shadow-sm">
            <CardHeader className="border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white pb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle className="text-xl font-semibold tracking-tight text-slate-900">{config?.createTitle ?? 'Create UOM'}</CardTitle>
                  <CardDescription className="mt-1 text-sm text-slate-600">Add a new unit of measure.</CardDescription>
                </div>
              </div>
            </CardHeader>
