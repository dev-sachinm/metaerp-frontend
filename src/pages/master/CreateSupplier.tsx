...
import { useCreateSupplier, type SupplierInput } from '@/hooks/graphql/useMasterDataMutations'
...
export function CreateSupplier() {
  ...
  const onSubmit = async (data: Record<string, unknown>) => {
    ...
    try {
      ...
      await create.mutateAsync(input as unknown as SupplierInput)
      logger.info('Supplier created', { category: 'business', data: { entity: ENTITY, code: input.code } })
      navigate(LIST_PATH, { replace: true })
    } catch (err: unknown) {
      logger.error('Create supplier failed', { category: 'technical', error: err })
      setFormError(getErrorMessage(err, 'Failed to create supplier'))
    }
  }
          ...
          <Card className="overflow-hidden border-slate-200/80 shadow-sm">
            <CardHeader className="border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white pb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle className="text-xl font-semibold tracking-tight text-slate-900">{config?.createTitle ?? 'Create Supplier'}</CardTitle>
                  <CardDescription className="mt-1 text-sm text-slate-600">Add a new supplier.</CardDescription>
                </div>
              </div>
            </CardHeader>
