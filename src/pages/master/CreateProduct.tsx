...
import { useCreateProduct, type ProductInput } from '@/hooks/graphql/useMasterDataMutations'
import { useProductCategories, useUOMList } from '@/hooks/graphql/useMasterDataQueries'
...
export function CreateProduct() {
  ...
  const onSubmit = async (data: Record<string, unknown>) => {
    ...
    try {
      ...
      await create.mutateAsync(input as unknown as ProductInput)
      logger.info('Product created', { category: 'business', data: { entity: ENTITY, name: input.name } })
      navigate(LIST_PATH, { replace: true })
    } catch (err: unknown) {
      logger.error('Create product failed', { category: 'technical', error: err })
          ...
          <Card className="overflow-hidden border-slate-200/80 shadow-sm">
            <CardHeader className="border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white pb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle className="text-xl font-semibold tracking-tight text-slate-900">{config?.createTitle ?? 'Create Product'}</CardTitle>
                  <CardDescription className="mt-1 text-sm text-slate-600">Add a new product.</CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="pt-6">
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                  ...
                  <FieldGuard entity={ENTITY} fieldName="categoryId" action="write">
                    <FormField
                  ...
                  <FieldGuard entity={ENTITY} fieldName="unitId" action="write">
                    <FormField
                      control={form.control}
                      name={'unitId'}
                      render={({ field }) => (
