...
function renderInput(
  ...
  const { type, placeholder, label } = fieldConfig
...
export function EntityFormFields<T extends FieldValues>({
  ...
  const permissionKey = permissions?.entities ? resolvePermissionEntityKey(entity, permissions.entities) : entity
  const entityFields = permissions?.entities?.[permissionKey]?.fields

  return (
    <section className="grid gap-4 md:grid-cols-2">
      {fields.map((fieldConfig) => {
        if (entity === 'product_category' && fieldConfig.key === 'parentId') {
          return null
        }
        if (entity === 'product' && fieldConfig.key === 'categoryId') {
          return null
        }
        if (entity === 'product' && fieldConfig.key === 'unitId') {
          return null
        }
        if (entity === 'expense_category' && fieldConfig.key === 'parentId') {
          return null
        }
        const canEdit =
          mode === 'create'
        ...
        return (
          <div key={fieldConfig.key} className={colSpanClass}>
          <FieldGuard
            entity={entity}
